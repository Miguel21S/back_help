...
///////////////////////   METHOD RETURNING LIST OF EMPLOYEES
...
const updateEmployee = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const employee_id = parseId(req.params.id, "employee")
        ...
        if (isActive !== undefined) {
            if (typeof (isActive) !== 'string') {
                throw new badRequestError("isActive must be a 'Active' or 'Inactive'");
            }
       ...
       const employee = await foundEntity<Employees>(
         Employees,
         { id: employee_id },
         "Employed not found",
         manager
        ...
        if (faculty_id !== undefined) {
            const facul_id = parseId(faculty_id, "faculty")
            await foundEntity<Faculty>(Faculty, { id: facul_id }, "faculty not found")
            id_facu = facul_id
        }

        if (dpto_academic_id !== undefined) {
            const dpto_acad_id = parseId(dpto_academic_id, "academic");
